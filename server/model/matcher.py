...
from .embeddings import EmbeddingGenerator  # Uses CodeBERT now
import logging
import os
...
class IssueMatcher:
    def __init__(self):
        self.cache = Cache()
        self.embedding_generator = EmbeddingGenerator()  # Now uses CodeBERT
        self.max_workers = 5
        self.semaphore = asyncio.Semaphore(10)  # Limit concurrent requests

    async def download_file_content(self, session, file):
        """Download file content asynchronously with rate limiting."""
    ...
    async def fetch_all_files(self, files):
        """Fetch all file contents asynchronously."""
    ...
    def calculate_similarity(self, vec1: torch.Tensor, vec2: torch.Tensor) -> float:
        """Compute cosine similarity between two tensors."""
        try:
            return float(torch.nn.functional.cosine_similarity(vec1, vec2).item())
        except Exception:
            return 0.0

    async def match_files(self, issue_data: Dict, filtered_files: List[Dict]) -> Dict:
        """Match files to the issue based on similarity scores using CodeBERT embeddings."""
        try:
            # Check cache first
            ...
            # Process issue text
            issue_text = f"{issue_data['title']} {issue_data.get('description', '')}".strip()
            ...
            for file_data in file_embeddings:
                similarity = self.calculate_similarity(issue_embedding, file_data['embedding'])
            ...
            result = {"filename_matches": matches[:3]}
